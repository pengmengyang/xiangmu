 ...
 * 
 */
public class Deal_pai_bill_Activity extends BaseActivity implements OnClickListener{
	
	private RelativeLayout title_back_rl;//返回
	private  RelativeLayout title_add_rl;
	private  TextView title_text_tv;
	private Button deal_pai_pai_man_spinner;
	private ShowEmployeeSpinnerAdapter showEmployeeSpinnerAdapter;
	private ProgressDialog progressDialog;
	private String tag="DEALPAIBILLACTIVITY";
	private EditText deal_pai_scan_billway_et;//输入运单号
	private Button deal_pai_add_btn;//添加
	private Button deal_pai_scan_btn;//扫描
	private ListView deal_pai_show_sing_in_waybill_message_lv;
	private ShowWaybillNumberListviewAdaper showWaybillNumberListviewAdaper;
	private TextView deal_pai_count_tv;//统计
	private String consigneeman;
	private final static int SCANNIN_GREQUEST_CODE = 1;
	private ProgressDialog progressDialog2;
	private Button deal_pai_bill_submit_btn;
	private ListView pop_window_lv;
	private String BelongNet;
	private TextView deal_pai_bill_scan_show_name;//显示派单员
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.deal_pai_bill_activity_layout);
	...
	private void initView() {
		...
		title_text_tv.setText("交派件单");
		
		deal_pai_bill_submit_btn=(Button) findViewById(R.id.deal_pai_bill_submit_btn);
		deal_pai_pai_man_spinner=(Button) findViewById(R.id.deal_pai_pai_man_spinner);
		deal_pai_bill_scan_show_name=(TextView) findViewById(R.id.deal_pai_bill_scan_show_name);
		
		deal_pai_bill_submit_btn.setOnClickListener(this);
		deal_pai_pai_man_spinner.setOnClickListener(this);
		
		progressDialog=ProgressDialog.show(Deal_pai_bill_Activity.this, "温馨提示", "数据正在加载中,请稍后");
		
		deal_pai_scan_billway_et=(EditText) findViewById(R.id.deal_pai_scan_billway_et);
		deal_pai_add_btn=(Button) findViewById(R.id.deal_pai_add_btn);
		deal_pai_scan_btn=(Button) findViewById(R.id.deal_pai_scan_btn);
		deal_pai_show_sing_in_waybill_message_lv=(ListView) findViewById(R.id.deal_pai_show_sing_in_waybill_message_lv);
		showWaybillNumberListviewAdaper=new ShowWaybillNumberListviewAdaper(getApplicationContext());
		deal_pai_count_tv=(TextView) findViewById(R.id.deal_pai_count_tv);
		deal_pai_show_sing_in_waybill_message_lv.setAdapter(showWaybillNumberListviewAdaper);
		
		deal_pai_add_btn.setOnClickListener(this);
		deal_pai_scan_btn.setOnClickListener(this);
		monitorListview(deal_pai_show_sing_in_waybill_message_lv);
	...
	public void onClick(View v) {
		...
		case R.id.title_back_rl://点击返回
			finish();
			break;
		case R.id.deal_pai_add_btn://点击添加
			getData();
			break;
		case R.id.deal_pai_scan_btn://点击扫描
			scanInputWaybliiNumber();
			break;
		case R.id.deal_pai_pai_man_spinner://点击查询
			progressDialog=ProgressDialog.show(Deal_pai_bill_Activity.this, "温馨提示", "数据正在加载中,请稍后");
			/**请求得到所有员工*/
			requestGetAllEmployee(v);
			break;
		case R.id.deal_pai_bill_submit_btn://点击提交
			progressDialog2=ProgressDialog.show(Deal_pai_bill_Activity.this,"温馨提示","数据正在提交中,请稍后");
			submitDataToSQL();
			break;
		...
		VolleyRequestUtil.getNewInstanceVolleyRequestUtil(getApplicationContext()).PostRequestString(Config.ADDALLSCAN,new InterfaceVolleyRequest() {
			...
			public void onsResponse(String response) {
				...
				if(response.equals("扫描成功")){
					...
					deal_pai_count_tv.setText(showWaybillNumberListviewAdaper.getCount()+"");
...
public void getData(){
		...
		String waybillnumber=deal_pai_scan_billway_et.getText().toString();
		if(null !=waybillnumber && waybillnumber.length() > 0 ){
			consigneeman=deal_pai_bill_scan_show_name.getText().toString();
		...
		deal_pai_count_tv.setText(showWaybillNumberListviewAdaper.getCount()+"");
		deal_pai_scan_billway_et.setText("");
	...
	public void scanInputWaybliiNumber(){
		...
		intent.setClass(Deal_pai_bill_Activity.this,MipcaActivityCapture.class);
		...
		VolleyRequestUtil.getNewInstanceVolleyRequestUtil(getApplicationContext()).PostRequestString(Config.SelectNameAndNetByPhone,new InterfaceVolleyRequest() {
			...
			public void onsResponse(String response) {
				...
				for(EmployeeBean employeeBean:list){
					deal_pai_bill_scan_show_name.setText(employeeBean.getEP_Name());
		  ...
		  final PopupWindow popupWindow = new PopupWindow(contentView,
	        ...
	        pop_window_lv.setOnItemClickListener(new OnItemClickListener() {
				...
				public void onItemClick(AdapterView<?> parent, View view,
					...
					deal_pai_bill_scan_show_name.setText(selectItem);
					popupWindow.dismiss();
				}
			});
